...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        self.model_guided = GuidedDenoiserWithGrad(model_wrap, cond_fn)

        output = queue.SimpleQueue()

        def callback(info):
            if info['i'] % 50 == 0:
                denoised = self.model_guided.orig_denoised
        ...
        for i, out in enumerate(self.samples):
            filename = f'out_{i}.png'
            K.utils.to_pil_image(out).save(filename)
            yield Path(filename)
    
    def worker(self):
        x = torch.randn([1, 3, self.side_y, self.side_x], device=self.device) * self.sigmas[0]
        if self.init is not None:
            x += self.init
        self.samples = K.sampling.sample_heun(self.model_guided, x, self.sigmas, second_order=False, s_churn=20, callback=self.callback)
        self.success = True
